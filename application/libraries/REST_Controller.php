...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
     ...
     *
     */
    private function privateError($errcontent)
    {
        $con = mysql_connect("localhost","common","common") or die("can not connect error mysql server");
        ...
        $sql = "insert into errorlog values(null,'BLL',$errno,'".$errstr."','".$errfile."','".$errline."','".json_encode($errcontent)."','".$date."')";
        mysql_query($sql);
        mysql_close($con);
    }   
	...
	public function response($data = array(), $http_code = null)
	{
		...
		else
		{
            ...
            if(is_array($data))
            {
                ...
                {
                    if(1!=$data['status'])$this->privateError($data);
                }
            }
            */
		...
		{
			header('Content-Length: ' . strlen($output)+50);
		}
        echo $output;exit;
		exit($output);
	}
